...
if __name__ == "__main__":
    ...
    engine_white = LearningEngine(name = "white", weights = None, epsilon = 0.1) # White Engine we're training
    board = chess.Board()
    ...
    try:
        while wins + losses + draws < max_games:
            ...
            if total_games % 100 == 0:
                ...
                engine_white.save_weights(f"mf1_weights/weights_{total_games}.json")
                ...
                results_df.to_csv("mf1_results/results.csv", index=False)
                print("Results saved to mf1_results/results.csv")
                
                epsilon_history.append(engine_white.epsilon)
                epsilon_df = pd.DataFrame({"Epsilon": epsilon_history})
                epsilon_df.to_csv("mf1_epsilon/epsilon.csv", index=False)
                print("Epsilon history saved to mf1_epsilon/epsilon.csv")
                ...
                time_df.to_csv("mf1_results/elapsed_time.csv", index=False)
